...
export function EditScheduleModal({
  ...
  const { classes, teachers, deleteSchedule } = useScheduler();
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      // 1. Delete all old records in this group
      const deletePromises = (schedule.ids || [schedule.id]).map((id) =>
        deleteSchedule(id),
      );
      await Promise.all(deletePromises);

      // 2. Create new records for each selected teacher
      const createPromises = data.teacherIds.map((tId) =>
        createSchedule({
      ...
      console.error("Failed to update schedule group", error);
    }
  };
