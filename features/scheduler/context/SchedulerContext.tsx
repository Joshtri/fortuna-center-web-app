...
export function SchedulerProvider({
    ...
    return (
      ((schedulesData?.data as any[]) || [])?.map((s) => {
        ...
        return {
          ...
          teacherId: primaryTeacher?.id || s.teacherId,
          teacherName: primaryTeacher?.name || s.teacherName,
          teacherColor:
            primaryTeacher?.color || stringToColor(s.teacherId || "unknown"),
        } as ClassSchedule;
      }) || []
    );
  ...
  const updateSchedule = useCallback(
    async (id: string, updates: Partial<ClassSchedule>): Promise<boolean> => {
      ...
      try {
        const idsToUpdate = schedule.ids || [id];
        const promises = idsToUpdate.map((targetId) =>
          apiUpdateSchedule(targetId, updates),
        );
  ...
  const deleteSchedule = useCallback(
    async (id: string) => {
      try {
        const schedule = schedules.find((s) => s.id === id);
        const idsToDelete = schedule?.ids || [id];

        const promises = idsToDelete.map((targetId) =>
